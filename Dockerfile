...
# python:3.12-slim and `pip install --upgrade pip` leave ensurepip wheels and
# sometimes a CycloneDX/SPDX file that Trivy prefers over the runtime pins.
# Purge those copies after the final install, not before (pip restores them).
RUN pip install --no-cache-dir --upgrade 'pip>=26.1.2' \
    ...
    && find /usr /root /tmp -xdev \( \
         -name 'setuptools-70*' -o -name 'msgpack-1.1*' \
         -o -name '*.cdx' -o -name '*.cdx.json' \
