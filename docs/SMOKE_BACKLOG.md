...
| ID | Item | Confirmed |
|----|------|-----------|
| `win11-25h2-sccmjoin-cncomputers` | Win11 **25H2** DHCP+AD, Target OU `CN=Computers,DC=lab,DC=test`, ODJ off, join user `sccmjoin@lab.test`, `DOMAIN_JOIN_VALIDATE_OU=false` | 2026-08-18 — guestos-lab local 2.8.2 (`build_time` 2026-08-18T20:16:25Z) after specialize Path fix. Template `121`; clone `W25CN201831` VMID **123** `task_id` `f7149344-584c-408c-bb02-76847623bc4e`. **FAILURE** (soft join): `setup.done=ok`; DHCP `192.168.123.163`; `class=not_an_ou`. `join-diag`: `add_computer_oupath=yes` `runner=SYSTEM` `25H2` `26200.8894`. NetSetup: `Specified path 'CN=Computers,…' is not an OU`; LDAP create `0x2`; **Cannot retry downlevel**. Matches production `SCCMJoin` + 24H2. Earlier admit-400 on GHCR 2.8.2 without the kill-switch (`W25CN195331`). Clone left running. |
| `win11-25h2-sccmjoin-empty-ou` | Win11 **25H2** DHCP+AD, blank Target OU, ODJ off, join user `sccmjoin@lab.test` (not Domain Admin) | 2026-08-18 — guestos-lab template `121` `WIn1125H2-1`; clone `W25SJ193708` VMID **126** SUCCESS (`task_id` `b331dd93-2ac0-4b2e-b33c-b5f95ecb125c`). GuestOS 2.7.1; `join-path method=add-computer`; DHCP `192.168.123.160`; `DisplayVersion=25H2` build `26200.8894`. Account `CN=W25SJ193708,CN=Computers,DC=lab,DC=test`. NetSetup: LDAP bind OK, object missing, then SAMR/downlevel create into `CN=Computers` (`NetpJoinDomainOnDs` `0x0`); **no** “is not an OU” / NetJoin `0x2`. Does **not** reproduce production `SCCMJoin` + 24H2 `0x2` (that path needs explicit `-OUPath CN=Computers`). Clone left running. |
